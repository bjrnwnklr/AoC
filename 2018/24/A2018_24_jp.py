    ...
    while True:
        ...
        for u in units:
            ...
            if targets:
                ...
                #print('%d unit %s selects %d unit %s' % (u.side, u.id, u.target.side, u.target.id))
        ...
        for u in units:
            if u.target:
                ...
                if killed > 0:
                    any_killed = True
                u.target.n -= killed
               # print('%d group %s deals %d damage to %d group %s, killing %d units. %d units left' %
                #        (u.side, u.id, dmg, u.target.side, u.target.id, killed, u.target.n))
